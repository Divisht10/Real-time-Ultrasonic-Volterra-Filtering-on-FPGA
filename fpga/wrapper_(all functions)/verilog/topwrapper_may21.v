...
always @(posedge clk) begin
    ...
    wire sqrt_vld;   // FIX 1: was used but never declared
    ...
    // FIX 3: wrap if/else in begin/end so vld_out resets properly
    always @(posedge clk) begin
        if (!reset) begin
            vld_delay_pipe <= 3'b0;
            vld_out        <= 1'b0;
        end else begin
            vld_delay_pipe <= {vld_delay_pipe[1:0], sqrt_vld};
...
wire [CALC_INST-1:0] calc_valid;
    ...
    wire [10:0] rf_addr_temp [0:CALC_INST-1];
...
/*            initial begin
       ...
       for (int j = 0; j < 1024; j++) begin
            ...
            initial begin
               // synthesis translate_off
               if      (i == 0) $readmemh("rf_ch_0.mem", local_rf_mem);
               else if (i == 1) $readmemh("rf_ch_1.mem", local_rf_mem);
               else if (i == 2) $readmemh("rf_ch_2.mem", local_rf_mem);
               else if (i == 3) $readmemh("rf_ch_3.mem", local_rf_mem);
               else if (i == 4) $readmemh("rf_ch_4.mem", local_rf_mem);
               else if (i == 5) $readmemh("rf_ch_5.mem", local_rf_mem);
               else if (i == 6) $readmemh("rf_ch_6.mem", local_rf_mem);
               else if (i == 7) $readmemh("rf_ch_7.mem", local_rf_mem);
               else if (i == 8) $readmemh("rf_ch_8.mem", local_rf_mem);
               else if (i == 9) $readmemh("rf_ch_9.mem", local_rf_mem);
               // synthesis translate_on
            end
            ...
            wire [10:0] current_addr = rf_addr_temp[i];
...
module dot_product_wrapper_seq #(
    ...
    wire signed [2*W+24:0] y_out;  // FIX 2: was [2*W+16:0]=81b, must match dot_product_seq [2*W+24:0]=89b
    wire core_done;
...
module w1_matched_filter_wrapper #(
    ...
    parameter TAPS        = 139,   // FW length = 2 × 68 (68 zeros + 68 chirp samples)
    parameter L           = 1792,  // Axial samples per channel (Receive.endSample in MATLAB)
    parameter PAD_LEN     = 138,   // TAPS-1 leading zeros for causal linear convolution
    parameter TOTAL_IN    = 1930,  // L + PAD_LEN → full convolution length = 1927
...
beamforming #(
    ...
    .center_idx     ({1'b0, lat_idx}), // FIX 5: zero-extend 7-bit lat_idx to 8-bit center_idx port
    ...
    if (!rst_n) begin
        ...
        case (state)
            ...
            S_STORE_PIX: begin
                ...
                env22_bram[({ax_idx[9:0], 7'b0} + {10'b0, lat_idx})] <= z2_val[DATA_WIDTH-1] ? (-z2_val) : z2_val;
                env23_bram[({ax_idx[9:0], 7'b0} + {10'b0, lat_idx})] <= z3_val[DATA_WIDTH-1] ? (-z3_val) : z3_val;
                state <= S_NEXT_LAT;
            end
